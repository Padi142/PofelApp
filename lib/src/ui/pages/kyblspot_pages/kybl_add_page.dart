        ...
        child: BlocListener<KyblCreationBloc, KyblCreationState>(
          listener: (context, state) {
            if (state is KyblspotCreationData &&
                state.mode == creationState.SUCCESFULY_ADDED) {
              ScaffoldMessenger.of(context)
                  .showSnackBar(SnackBarAlert(context, 'Úspěšně přidáno pooog'));
            }
          },
          child: Flex(
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        onChanged: (value) {
                          if (value.length >= 3) {
                            BlocProvider.of<KyblCreationBloc>(context)
                                .add(AddKyblspotDesc(kyblDesc: value));
                          }
                        },
                        decoration: InputDecoration(
                          labelText:
                    ...
                    children: [
                      ...
                      BlocBuilder<KyblCreationBloc, KyblCreationState>(
                        builder: (context, state) {
                          ...
                          if (state.mode == creationState.POSITION_SPECIFIED) {
                            return Padding(
                              padding: const EdgeInsets.all(8.0),
                              child: Row(
                                children: [
                                  Text(
                                      "${state.position.latitude.toString().substring(0, 8)}  ${state.position.longitude.toString().substring(0, 8)}",
                                  ...
                                  TextButton(
                                      ...
                                      child: const Text(
                                        "Změnit",
                                        style:
                                            TextStyle(color: Color(0xFF8F3BB7)),
                                      ))
                                ],
                              ),
                            );
                          ...
                          return OutlinedButton(
                            onPressed: () {
                              if (state.mode == creationState.NAME_DESC) {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) =>
                            ...
                            child: const Padding(
                              ...
                              child: Text('Zadat místo',
                                  style: TextStyle(color: Color(0xFF8F3BB7))),
                            ),
                          );
                        },
                      )
                    ...
                    child: BlocBuilder<KyblCreationBloc, KyblCreationState>(
                      builder: (context, state) {
                        ...
                        return ElevatedButton(
                          onPressed: () {
                            if (state.mode == creationState.POSITION_SPECIFIED) {
                              BlocProvider.of<KyblCreationBloc>(context).add(
                                CreateKyblspot(
                                    state.name, state.description, state.position),
                              );
                              Navigator.pop(context);
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBarError(
                                      context, 'Získej nejdřív souřadnice'));
                            }
                          },
                          style: OutlinedButton.styleFrom(
